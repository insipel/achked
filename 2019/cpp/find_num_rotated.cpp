...
int find_num(int a[], int sz, int num)
{
    ...
    /*
     * If 'start' and 'end' are adjacency to each other, then check
     * the numbers in these 2 indices to declare the match. Again, for
     * number match, we would have to adjust with reset_point.
     */
    if ((r - p) == 1) {
        if (a[(r+start_idx)%sz] == num) return ((r+start_idx)%sz);
        if (a[(p+start_idx)%sz] == num) return ((p+start_idx)%sz);
    }
